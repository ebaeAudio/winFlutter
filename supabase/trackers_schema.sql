-- Custom Trackers schema (v1)
--
-- Tables:
-- - trackers: user-defined tracker definitions (3 items stored in jsonb)
-- - tracker_tallies: per-day counts per tracker item
--
-- Notes:
-- - RLS is enabled; policies restrict access to auth.uid().
-- - Deleting a tracker is "archive" (set archived=true); history is retained.
...
create table if not exists public.tracker_tallies (
  ...
  date text not null, -- YYYY-MM-DD (kept as text to match existing tasks schema)
